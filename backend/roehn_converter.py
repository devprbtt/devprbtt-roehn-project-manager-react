...
class RoehnProjectConverter:
    ...
    def process_db_project(self, projeto):
        ...
        for modulo in projeto.modulos:
            # Pular o controlador principal
            if modulo.id == main_controller_id:
                continue

            # Verificar se o módulo já foi processado (está em algum quadro)
            modulo_em_quadro = any(
                modulo in quadro.modulos 
                for area in projeto.areas 
                for ambiente in area.ambientes 
                for quadro in ambiente.quadros_eletricos
            )
    ...
    def _ensure_module_exists(self, model, module_name=None, automation_board_guid=None):
        ...
        # --- CORREÇÃO ---
        # Adicionar verificação para tipos de controladores, mas criando-os como módulos normais (Logicserver=False)
        if "AQL-GV-M4" in key:
            ...
            # Fallback para RL12 se o tipo não for reconhecido
    ...
    def _create_controller_as_module(self, controller_type, name, hsnet_address, dev_id, target_board=None):
        ...
        # Usa a mesma lógica de _create_controller_module, mas sobrescreve o Logicserver
        module_json = self._create_controller_module(controller_type, controller_info)
        
        # --- PONTO CRÍTICO DA CORREÇÃO ---
        module_json["Logicserver"] = False
        module_json["Name"] = name # Usar o nome do banco de dados, não o tipo
        module_json["Guid"] = str(uuid.uuid4()) # Garantir um GUID único

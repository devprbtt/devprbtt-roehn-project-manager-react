...
type Modulo = {
  id: number;
  nome: string;
  tipo: string; // RL12 | RL4 | LX4 | SA1 | DIM8
  canais_disponiveis: number[]; // fornecido pelo backend
  quantidade_canais?: number;   // opcional (informativo)
};

type Vinculacao = {
  id: number;
  circuito_id: number;   // <<--- NOVO
  identificador: string;
...
type OptionsPayload = {
  compat: Record<"luz" | "persiana" | "hvac", string[]>; // ex: { luz: ["RL12","RL4","DIM8"], ... }
  circuitos: Circuito[];
  modulos: Modulo[];
};
...
export default function Vinculacao() {
  ...
  const circuitoSelecionado = useMemo(
    () => options.circuitos.find(c => c.id === circuitoId),
    [circuitoId, options.circuitos]
  );
  ...
  useEffect(() => {
    if (projetoSelecionado) fetchAll();
  }, [projetoSelecionado]);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (res.ok && (data?.ok || data?.success)) {
        ...
        setModuloId("");
        setCanal("");
        // recarrega listas e opções (para atualizar canais livres)
        await fetchAll();
  ...
  async function handleDelete(id: number) {
    ...
    try {
      ...
      if (res.ok && (data?.ok || data?.success)) {
        ...
        // recarrega opções para devolver o canal como livre
      ...
      <div className="max-w-6xl mx-auto">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-2xl font-bold flex items-center gap-2">
            <Plug className="h-5 w-5" /> Vincular Circuitos a Módulos
          </h2>
          <p className="text-sm text-muted-foreground">Associe seus circuitos a módulos e canais disponíveis.</p>
        </div>
        ...
        <Card className="border-0 shadow-sm rounded-3xl mb-4">
          <CardHeader className="border-0">Vincular Circuito</CardHeader>
          <CardContent>
        ...
        <Card className="border-0 shadow-sm rounded-3xl">
          <CardHeader className="border-0">Vinculações Existentes</CardHeader>
          <CardContent>
            {loading ? (
                  ...
                  <tbody>
                    {vinculacoes.map(v => (
                      <tr key={v.id} className="border-b">
                        <td className="py-2"><strong>{v.identificador}</strong> — {v.circuito_nome}</td>
                        <td className="py-2">{v.area_nome}</td>
                        <td className="py-2">{v.ambiente_nome}</td>
                        <td className="py-2">{v.modulo_nome} ({v.modulo_tipo})</td>

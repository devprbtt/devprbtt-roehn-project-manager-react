...
import { Printer, FileDown, FileOutput, Lightbulb, Blinds, Snowflake, LayoutList } from "lucide-react";
...
export default function Projeto() {
  ...
  useEffect(() => { if (projetoSelecionado) fetchTree(); }, [projetoSelecionado]);
  ...
  return (
    <Layout projectSelected={projetoSelecionado}>
      <div className="min-h-screen bg-background print:bg-white">
        <div className="container mx-auto px-4 py-8 print:max-w-full">
          {/* Header no padrão */}
            ...
            <div className="flex items-center gap-3 mb-2">
              ...
              <div>
                <h1 className="text-3xl font-bold text-foreground">
                  Visualizar Projeto Completo{data.projeto?.nome ? `: ${data.projeto.nome}` : ""}
                </h1>
                <p className="text-muted-foreground mt-1">Resumo por área, ambiente e circuito.</p>
              </div>
            </div>
          ...
          {loading ? (
            ...
            <div className="space-y-6">
              {data.areas.map(area => (
                <Card key={area.id} className="border-0 shadow-sm rounded-3xl bg-card/50 backdrop-blur-sm">
                  <CardHeader className="border-0 pb-2">
                    <CardTitle className="text-xl">Área: {area.nome}</CardTitle>
                  </CardHeader>
                  <CardContent>
      ...
      {/* Modal .rwp (mantido, apenas organização visual) */}
      {showRwp && (
